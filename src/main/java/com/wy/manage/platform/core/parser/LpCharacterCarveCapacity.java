 ...
 * Created by tianye
 */
public class LpCharacterCarveCapacity implements CharacterCarveCapacity{
        ...
        if(stack.empty()){
            ...
            if(specialCclStart.size()>0){
                ...
                if(peek.getMeanType()==MeanType.CHANGE_MEANING
                    ...
                    NfaStateMachine characterRepertoireNfaStateMachine = NfaManager.createCharacterRepertoireNfaStateMachine(nfaStateMachine,SymbolType.LP.getState());
                ...
                ExceptionTools.ThrowException("{不应该换行");
            }else {
                xContentItemL.setMeanType(MeanType.NO_CHANGE_MEANING);
                XContentItem pop = stack.pop();
                ...
                if(pop.getNfaStateMachine()==null){
                    stack.push(pop);
                    linkNfaStateMachine=NfaManager.createSingleCharacterNfaStateMachine(SymbolType.LP.getState());
                }else {
                    linkNfaStateMachine = NfaManager.createLinkNfaStateMachine(pop.getNfaStateMachine(), NfaManager.createSingleCharacterNfaStateMachine(SymbolType.LP.getState()));
                }
